...
didSelectPiece = False #global variable that tracks whether a piece is currently selected
...
destinationSquare = [] # a global instance that tracks the square that a piece attempts to go to 
...
def draw():
    ...
    fill(255, 255, 255)
    stroke(0)
    rect(400, 0, 200, 400) #creates a white background for the score side of the interface
    fill(0,0,0)
    redsLeft = "Reds Left: " + str(numRed)# creates text showing how many reds are left
    greensLeft = "Greens Left: " + str(numGreen) #creates text showing how many greens are left
...
def mouseClicked():
    ...
    if(player == 1):
        ...
        for i in range(0, numGreen, 1):#does the same thing as the previous loop except for player two
            ...
            for i in range(8):
                for j in range(8):
                    if(moveMade):#ends the inner for loop if the move is made
                        ...
                        makeMove(i, j)#checks to see if the move can be made for the current square 
            ...
            for i in range(8):
                for j in range(8):
                    if(moveMade):#ends the inner for loop if the move is made
                        ...
                        makeMove(i, j)#checks whether or not the move can be made for the current i,j board square
                if(moveMade):
...
def makeBoard():#loops through and creates a board that is an 8x8 array with alternating colors of blue and white
    ...
    for i in range(8):
        x = 0
        for j in range(8):
            if(i%2 == 0 and j%2 == 0 or (i%2 == 1 and j% 2 == 1) ):#alternates the colors in a checkered pattern
                fill(255, 255, 255)
            else:
                fill(0, 0, 255)
            boardSquare = BoardSquare(x, y)#creates a new instance of type boardsquare with the specified x and y coordinates 
            boardSquare.buildSquare()
            board.append(boardSquare)#appends the boardsquare to the array of boardsquares that makeup the board
            setUpPieces(i, j, x, y)#calls the set up pieces function to lay out and create all of the pieces
            x += 50
        y += 50
...
def setUpPieces(i, j, x, y):
    ...
    if(i < 3 and ((j % 2 == 0 and i % 2 == 0) or (j % 2 == 1 and i % 2 == 1))):
        fill(255,0,0)
        piece = Piece("checker", 25 + x, 25 + y)#creates a new instance of type piece,
...
def correctSelectionOfPiece(piece):# returns whether or not a mouse is clicked on a correct piece 
    x = getattr(piece, 'x')
    y = getattr(piece, 'y')
    return ((mouseX >= x - 25 and mouseX <= x + 25) and 
                (mouseY >= y - 25 and mouseY <= y + 25))

def highlightSelectedPiece(piece):#highlights the selected piece by rebuilding it with a gold stroke
    x = getattr(piece, 'x')
    y = getattr(piece, 'y')
...
def dehighlightSelectedPiece(selectedPiece):#dehighlights the selected piece when called 
    global redPieces
    global greenPieces
...
def makeMove(i, j):# checks if the player's mouse is a valid square and moves the piece accordingly
    ...
    if(player == 1):
        ...
        if(isValidDestination(x, y, selectedPiece, player)):#if the square is a valid
            ...
            fill(255,0,0)
            if(getattr(selectedPiece, 'name') == "king" or y == 350):  #checks if its a king or its becoming a king
            ...
            selectedPiece = redPieces[index]#updates all of the global variables that are tracking information
            ...
            updateTile(selectedPiece)
            fill(255,0,0)
            if(getattr(selectedPiece, 'name') == "king" or y == 350): #checks if its a king or becoming a king 
                redPieces.insert(index, Piece("king", x + 25, y + 25))
                redPieces.remove(selectedPiece)
                redPieces[index].buildKing()  
            else:
                ...
                redPieces.remove(selectedPiece)
                redPieces[index].buildPiece()
            selectedPiece = redPieces[index]#updates global variables tracking information
            didSelectPiece = False
            player = 2
            moveMade = True
...
def isValidDestination(x, y, selectedPiece, player):#checks to see whether or not the mouse is clicked on a tile and the pice can move left or right 
    return ((isCorrectSquare(x, y)) and (canMoveLeftOrRight(x, y, selectedPiece, player)))

def isCorrectSquare(x, y):#checks to see if the mouse x and y coordinates are within a board square
...
def isSpotOccupied(x, y, pieces):#returns whether or not a spot is occupied by the given pieces array 
    ans = False
...
def canTakePiece(x, y, selectedPiece, player):# returns whether or not the square selected 
    ...
    if(player == 1 and name == "king" and (not isSpotOccupied(x, y, greenPieces)) and ((y == pieceY + 75) or (y == pieceY - 125))):
        ...
        if(checkDownLeft(x, pieceX, pieceY, greenPieces)):
            return removeGreenPiece(pieceX - 50, pieceY + 50)#Removes the piece that the selected piece jumps over
...
def checkUpRight(x, pieceX, pieceY, pieces):#returns whether or not the piece can move two to the right and if the spot up and to the right is occupied
    return (x == pieceX + 75 and isSpotOccupied(pieceX + 25, pieceY -75, pieces))

def checkUpLeft(x, pieceX, pieceY, pieces):#returns whether or not the piece can move two to the left and if the spot up and to the left is occupied
    return (x == pieceX - 125 and isSpotOccupied(pieceX - 75, pieceY - 75, pieces))

def checkDownRight(x, pieceX, pieceY, pieces):#returns whether or not the piece can move two to the right and if the spot below and to the right is occupied
    return (x == pieceX + 75 and isSpotOccupied(pieceX + 25, pieceY + 25, pieces))

def checkDownLeft(x, pieceX, pieceY, pieces):#returns whether or not the piece can move two to the left and if the spot down and to the left is occupied
